...
def parse_session_text(session_text):
    ...
    try:
        ...
        # Extract time (handles formats like "9:00-12:00 pm", "9:00am-12:00pm", "2:00-3:00pm", "12:30pm–1:30pm")
        ...
        # Try MM/DD/YY format first (also handle comma after date)
        date_match = re.match(r'(\d{1,2})/(\d{1,2})/(\d{2})', date_part)
        if date_match:
            month, day, year = date_match.groups()
            date_iso = f"20{year}-{int(month):02d}-{int(day):02d}"
            ...
            if month_match:
                ...
                # Get current year or next year if past
                current_year = datetime.now().year
                try:
                    test_date = datetime.strptime(f"{month_name} {day} {current_year}", "%B %d %Y")
                    if test_date < datetime.now():
                        current_year += 1
                    date_iso = f"{current_year}-{test_date.month:02d}-{int(day):02d}"
                except ValueError:
                    pass
